...
const login = async (req, res) => {
    try {
        ...
        if (!rollno || !regno || !name) {
            return res.status(400).json({ error: "Name, roll number, and registration number are required" });
        }
        ...
        const user = await StudentModel.findOne({ rollno, regno, name: formattedName });
